...
const COURSE_MODES = ["INTEGRATED", "NON_INTEGRATED", "FINAL_SUMMARY", "NCMC"] as const;
...
const BaseCourseSchema = z.object({
  ...
  name: z.string().min(1, "Course name is required").max(200, "Course name must be less than 200 characters").trim(),
  ...
  semesterNumber: z.number().int().min(1, "Semester number is required").max(8, "Semester number must be between 1 and 8"),
  ...
  allowFeedback: z.boolean().default(false),
  attendanceRequired: z.boolean().default(true),
});

const MODE_LOCKED_VALUES = {
  INTEGRATED: { tutorialCredits: 0, skillCredits: 0 },
  NON_INTEGRATED: { tutorialCredits: 0, practicalCredits: 0, skillCredits: 0, labCount: 0, labMaxMarks: 0 },
  FINAL_SUMMARY: { tutorialCredits: 0, practicalCredits: 0, skillCredits: 0, labCount: 0, labMaxMarks: 0, aatMaxMarks: 0, theoryMaxMarks: 0 },
  NCMC: { tutorialCredits: 0, practicalCredits: 0, skillCredits: 0, seeMaxMarks: 0, cieMaxMarks: 0, theoryMaxMarks: 0, labMaxMarks: 0, aatMaxMarks: 0 },
} as const;

type ModeName = keyof typeof MODE_LOCKED_VALUES;
type ModeLockedField = keyof (typeof MODE_LOCKED_VALUES)[ModeName];

const addLockedFieldIssue = (ctx: z.RefinementCtx, field: ModeLockedField, expected: number, mode: ModeName) => {
  ctx.addIssue({ code: z.ZodIssueCode.custom, path: [field], message: `${field} must be ${expected} for ${mode} mode` });
};

const validateModeLockedValues = (value: Record<string, unknown>, mode: ModeName, ctx: z.RefinementCtx, strictMissing = false) => {
  const lockedValues = MODE_LOCKED_VALUES[mode];
  (Object.entries(lockedValues) as Array<[ModeLockedField, number]>).forEach(([field, expected]) => {
    const provided = value[field as string];
    if (provided === undefined) {
      if (strictMissing) addLockedFieldIssue(ctx, field, expected, mode);
      return;
    }
    if (typeof provided !== "number" || provided !== expected) addLockedFieldIssue(ctx, field, expected, mode);
  });
};
export const CreateCourseSchema = BaseCourseSchema.superRefine((value, ctx) => {
  if (!value.departmentId && !value.departmentName) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["departmentId"], message: "departmentId or departmentName is required" });
  }
  validateModeLockedValues(value as unknown as Record<string, unknown>, value.courseMode, ctx, true);
});
...
export const UpdateCourseSchema = BaseCourseSchema.partial().extend({
  ...
  validateModeLockedValues(value as unknown as Record<string, unknown>, value.courseMode, ctx, false);
});

export const DeleteCourseSchema = z.object({ id: z.string().uuid("Course ID is required") });

const ApprovalScopeSchema = z.object({
  semesterId: z.string().uuid("Invalid semester ID"),
  departmentId: z.string().uuid("Invalid department ID").optional(),
  departmentName: z.string().min(1, "Department is required").optional(),
  cycle: z.enum(COURSE_CYCLES).or(z.literal("")).transform((value) => (value === "" ? undefined : value)).optional(),
});

const requireDepartmentScope = <T extends { departmentId?: string; departmentName?: string }>(value: T, ctx: z.RefinementCtx) => {
  if (!value.departmentId && !value.departmentName) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["departmentId"], message: "departmentId or departmentName is required" });
  }
};
...
export const ApproveSemesterCoursesSchema = ApprovalScopeSchema.superRefine((value, ctx) => { requireDepartmentScope(value, ctx); });
export const RequestRevisionForSemesterSchema = ApprovalScopeSchema.extend({ reviewerNotes: z.string().trim().min(1, "Revision notes are required") }).superRefine((value, ctx) => { requireDepartmentScope(value, ctx); });
