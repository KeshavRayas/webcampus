...
// FIX: Subarno - Changed the columns to display only name, emmployeeId, staffType, official email, designation and mobile number. Also added department column when all departments are selected. The rest of the details can be viewed in the faculty details page. This is done to reduce the clutter in the table and make it more user friendly.

export const getAdminFacultyColumns = (
  selectedDepartmentId: string,
  ALL_DEPARTMENTS_VALUE: string
): ColumnDef<AdminFacultyResponse>[] => {
  const columns: ColumnDef<AdminFacultyResponse>[] = [
    {
    ...
    },
    {
      ...
      header: "Designation",
      cell: ({ row }) => (
        <div>{formatDesignation(row.original.designation)}</div>
      ),
    },
    {
