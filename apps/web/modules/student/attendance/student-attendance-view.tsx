...
import { FilterActions, FilterBuilder, FilterFieldConfig, FilterPanel } from "@webcampus/ui/components/filter-builder";
...
export const StudentAttendanceView = () => {
  ...
  const [draftFilters, setDraftFilters] = useState<AttendanceFilters>({ termId: "", semesterId: "" });
  const [appliedFilters, setAppliedFilters] = useState<AttendanceFilters>({ termId: "", semesterId: "" });
  ...
  const filterFields: FilterFieldConfig<AttendanceFilters>[] = [
    {
      ...
      label: "Academic Term",
      type: "select",
      options: terms?.map((t) => ({ label: `${t.type} ${t.year}`, value: t.id })) || [],
    ...
    },
    {
      ...
      label: "Semester",
      type: "select",
      options: selectedTermSemesters.map((s) => ({ label: `Semester ${s.semesterNumber}`, value: s.id })),
      placeholder: draftFilters.termId ? "Select Semester" : "Select Term First",
    },
  ];
  ...
  const { data: summaries, isLoading: summaryLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axios.get<BaseResponse<CourseAttendanceSummary[]>>(
        ...
        { params: { semesterId: appliedFilters.semesterId }, withCredentials: true }
      );
  ...
  const { data: details, isLoading: detailsLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axios.get<BaseResponse<{ course: { code: string; name: string }; sessions: SessionDetail[] }>>(
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">{selectedCourse.name} - Attendance Details</h1>
        ...
        {detailsLoading ? (
          ...
          <div className="rounded-lg border p-8 text-center text-muted-foreground">No attendance sessions recorded yet for this course.</div>
        ) : (
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Track your academic attendance course-wise across semesters.</p>
      </div>
      ...
      {appliedFilters.semesterId && (
        <div className="space-y-6">
          {summaryLoading ? (
            ...
            <div className="rounded-lg border bg-muted/20 p-8 text-center">No registered courses found for this semester.</div>
          ) : (
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {summaries.map((item) => (
                <Card
                  ...
                  onClick={() => setSelectedCourse({ id: item.courseId, name: item.courseName })}
                >
                  <CardHeader className="bg-muted/10 border-b pb-3">
                    <CardTitle className="flex items-start justify-between text-lg">
                      <span className="truncate pr-2" title={item.courseName}>{item.courseName}</span>
                  ...
                  <CardContent className="space-y-4 pt-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`text-2xl font-bold ${item.percentage >= 75 ? "text-green-600" : "text-amber-600"}`}>
                        {item.percentage}%
                      </span>
                    </div>

                    <div className="text-xs text-muted-foreground flex justify-between border-t pt-3">
                      <span>Attended: {item.attendedClasses} / {item.totalClasses} classes</span>
