...
export class CoeService {
  ...
  static async create(
    ...
    try {
      ...
      if (user.status === "error" || !user.data || !user.data.id) {
        throw new Error(user.message || "Failed to create user");
      }

      const newUserId = user.data.id as string;
  ...
  // ADDED NEW UPDATE METHOD - Subarno
  ...
  static async update(
    ...
    try {
      ...
      // Cleans up the old photo from S3 to save storage
      if (uploadedImageUrl && existingCoe.user.image) {
        try {
          await this.deletePhoto(existingCoe.user.image);
        } catch (cleanupError) {
      ...
      // Returns the full object so the frontend UI updates instantly
      return {
        status: "success",
        message: "COE updated successfully",
        data: {
      ...
      if (uploadedImageUrl) {
        try {
          ...
          logger.warn("Failed to clean up new COE photo on error", {
            uploadedImageUrl,
            cleanupError,
          });
        }
